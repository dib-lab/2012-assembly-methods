...
environments pose new challenges to sequence analysis approaches.  \emph{De 
novo} metagenomic assembly effectively reduces the total amount of data to be 
analyzed but requires significant computational resources and removal of low 
abundance sequences, biasing against rare species.  Using two approaches, 
digital normalization and partitioning, large metagenomic datasets can effectively 
be reduced and divided for tractable assembly.  Using a human gut mock community 
dataset, we demonstrate that these methods result in assemblies nearly identical to 
assemblies from unprocessed data.  The application of these methods on two large 
soil metagenomes resulted in assemblies in under 14 GB of memory and less than 
a day.  The predicted functional and phylogenetic content of the assembled contigs 
indicate significant phylogenetic differences though the functions encoded in both 
soil datasets were similar.  The strategies presented are generic and can be extended 
to any metagenome to provide partitions of reads which can be efficiently assembled or 
otherwise analyzed. 
...
reads and contigs aligning to distinct genomes (Supp. Fig. X). (B) The genomic content of all assemblies were 
