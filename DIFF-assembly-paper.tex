...
%DIF < \author{Adina Chuang Howe$^{1,2}$, 
%DIF -------
\author{Adina Chuang Howe$^{1,2}$, Janet Jansson$^{3,4}$, Stephanie A. %DIF > 
%DIF < Janet Jansson$^{3,4}$,
Malfatti$^{3}$, Susannah G. Tringe$^{3}$, James M. Tiedje$^{1,2}$, and C. Titus %DIF > 
%DIF < Stephanie A. Malfatti$^{3}$,
Brown$^{1,5\ast}$}  %DIF > 
%DIF < Susannah G. Tringe$^{3}$,
%DIF < James M. Tiedje$^{1,2}$, and 
%DIF < C. Titus Brown$^{1,5\ast}$}
...
Mackelprang:2011p1087,Qin:2010p189,Tringe:2005p174,Venter:2004p170}. \DIFdelbegin \DIFdel{These systemsare }\DIFdelend \DIFaddbegin \DIFadd{DNA
sequencing has begun to reveal the enormous diversity and heterogeneity
associated within these systems, making them }\DIFaddend difficult to study {\em in situ}
\DIFdelbegin \DIFdel{, and we are only
beginning to understand their diversity and functional potential. Advances in DNA sequencingnow provide }\DIFdelend \DIFaddbegin \cite{Hess:2011p686,Mackelprang:2011p1087,Qin:2010p189}\DIFadd{. With ultradeep
sequencing, we now have }\DIFaddend unprecedented access to \DIFdelbegin \DIFdel{the genomic content of these communities via shotgun
sequencing, which produces millions to billions of short-read
sequences }%DIFDELCMD < \cite{Hess:2011p686,Mackelprang:2011p1087,Qin:2010p189}%%%
\DIFdel{.
Because shotgun sequencing samples communities randomly, ultradeep
sequencing is needed to detect }\DIFdelend \DIFaddbegin \DIFadd{even the }\DIFaddend rare species in \DIFdelbegin \DIFdel{environmental samples, with an estimated }\DIFdelend \DIFaddbegin \DIFadd{these
environment (i.e., ~}\DIFaddend 50 Tbp \DIFdelbegin \DIFdel{needed for an individual }\DIFdelend \DIFaddbegin \DIFadd{required to adequately sample a }\DIFaddend gram of soil
\cite{Gans:2005p1365}\DIFdelbegin \DIFdel{.  Both short read lengths and the large volume
of sequencing data pose }\DIFdelend \DIFaddbegin \DIFadd{).
}

\DIFadd{Alongside sequencing breakthroughs, }\DIFaddend new challenges to sequence analysis
approaches \DIFdelbegin \DIFdel{.  }\DIFdelend \DIFaddbegin \DIFadd{have emerged. Sequencing dataset sizes are growing at an exponential
rate, requiring significant computational resources for data storage and
analysis. }\DIFaddend A single metagenomic project can readily generate as much or more
...
\DIFaddbegin \DIFadd{Consequently, investigators of metagenomic datasets are confounded by
...
genomes). }\DIFaddend \emph{De novo} assembly of \DIFdelbegin \DIFdel{raw }\DIFdelend sequence data offers several advantages
\DIFdelbegin \DIFdel{over analyzing the sequences directly. Assembly removes }\DIFdelend \DIFaddbegin \DIFadd{for analyzing metagenomic datasets. It provides improved accuracy of sequences by
removing }\DIFaddend most random sequencing errors and \DIFdelbegin \DIFdel{decreases the total amount of data to be
analyzed, yielding assembled }\DIFdelend \DIFaddbegin \DIFadd{results in }\DIFaddend contigs longer and more
specific than \DIFaddbegin \DIFadd{unassembled }\DIFaddend sequencing reads. \DIFaddbegin \DIFadd{Further, assembly significantly
reduces the total volume of data required for downstream analysis (e.g., gene
annotation). }\DIFaddend Importantly, \emph{de novo} assembly \DIFaddbegin \DIFadd{also }\DIFaddend does not rely on the
...
to a human gut mock community \DIFaddbegin \DIFadd{(HGMC) }\DIFaddend dataset, and find that these filtering
...
assembly \DIFaddbegin \DIFadd{of the HGMC dataset }\DIFaddend by comparing unfiltered traditional assembly to
the the described filtered assembly (Fig. \ref{flowchart}; see Methods and
Supplementary Information). \DIFdelbegin \DIFdel{Initially, the }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend abundance of genomes within the \DIFdelbegin \DIFdel{mock
}\DIFdelend \DIFaddbegin \DIFadd{HGMC }\DIFaddend dataset
...
\DIFdelbegin \DIFdel{2 and }\DIFdelend \DIFaddbegin \DIFadd{2and }\DIFaddend 3). Overall, the unfiltered dataset reads covered a total of 93\% of the
reference genomes. \DIFdelbegin \DIFdel{Filtering removed }\DIFdelend \DIFaddbegin \DIFadd{Reads were removed based on their coverage within the
dataset (See Methods); we removed a total of }\DIFaddend 5.9 million reads \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{(}\DIFaddend 40\% of the total
\DIFaddbegin \DIFadd{reads) from the original HGMC dataset }\DIFaddend (Table ~\ref{data-summary})\DIFdelbegin \DIFdel{;  the remaining
reads }\DIFdelend \DIFaddbegin \DIFadd{. The remaining
reads in the filtered HGMC dataset }\DIFaddend covered 91\% of the reference genomes (Table
~\ref{data-summary} and Supplementary Fig. 2 and 3).

We next compared the recovery of reference genomes \DIFdelbegin \DIFdel{in }\DIFdelend \DIFaddbegin \DIFadd{from }\DIFaddend contigs assembled from
the original and filtered \DIFaddbegin \DIFadd{HGMC }\DIFaddend datasets. Using the Velvet assembler
\DIFaddbegin \cite{Zerbino:2008p665}\DIFaddend ,we recovered 43\% and 44\% of the reference genomes,
...
(SOAPdenovo \DIFaddbegin \cite{Li:2010jz} \DIFaddend and Meta-IDBA \DIFaddbegin \cite{Peng:2011p898} \DIFaddend ,
Table~\ref{assembly-summary}). Overall, the unfiltered and filtered assemblies
were very similar, sharing 95\% of genomic content. \DIFdelbegin \DIFdel{For the highest abundance }\DIFdelend \DIFaddbegin \DIFadd{In the most abundant
}\DIFaddend references (the plasmids NC\_005008.1, NC\_005007.1, and NC\_005003.1), the
...
We \DIFdelbegin \DIFdel{estimated the abundance of assembled contigs and reference genomes
using the mapped sequencing reads}\DIFdelend \DIFaddbegin \DIFadd{compared the estimated abundance of genomes in the HGMC dataset using reads
...
the median base pair coverage of all aligned reads. }\DIFaddend (Supplementary Fig. 5). \DIFdelbegin \DIFdel{Above a
sequencing coverage of five}\DIFdelend \DIFaddbegin \DIFadd{For
assembled contigs with coverage greater than 5}\DIFaddend , the majority of reads which
could be \DIFaddbegin \DIFadd{aligned contigs were also }\DIFaddend mapped to reference genomes \DIFdelbegin \DIFdel{were included in the assembled contigs
}\DIFdelend (Supplementary
Fig. 3 and 4). Below this threshold, reads \DIFdelbegin \DIFdel{could be }\DIFdelend \DIFaddbegin \DIFadd{were }\DIFaddend mapped to reference genomes but
were less likely to be associated with assembled contigs. \DIFdelbegin \DIFdel{We next compared the
abundances of the reference genomes to
the abundances of the contigs in the }\DIFdelend \DIFaddbegin \DIFadd{Comparing the
}\DIFaddend unfiltered and filtered assemblies\DIFdelbegin \DIFdel{.  The abundance estimations }\DIFdelend \DIFaddbegin \DIFadd{, the estimated abundance of the HGMC genomes
}\DIFaddend from the filtered assembly were significantly closer to predicted abundances
...
The \DIFdelbegin \DIFdel{resulting assemblies of
unpartitioned and partitioned were more than 99}%DIFDELCMD < \% %%%
\DIFdel{identical.  In the
mock dataset , we identified 9 million reads in }\DIFdelend \DIFaddbegin \DIFadd{HGMC dataset was partitioned into }\DIFaddend 85,818 disconnected partitions \DIFaddbegin \DIFadd{containing
a total of 9 million reads }\DIFaddend (Supplementary Fig. 6). Among these, only 2,359
(2.7\%) of the partitions contained reads originating from more than one
genome,indicating that partitioning separated reads from distinct species. \DIFaddbegin \DIFadd{The
resulting assemblies of the unpartitioned and partitioned dataset were very
similar, sharing 99}\% \DIFadd{identical genomic content.
}\DIFaddend 

In general, reference genomes with high sequencing coverage were associated
with fewer partitions (Supplementary Table 1)\DIFdelbegin \DIFdel{: }\DIFdelend \DIFaddbegin \DIFadd{; }\DIFaddend a total of 112 partitions
contained
reads from high abundance reference genomes (coverage above 25) compared to
2,771 partitions associated with lower abundance genomes \DIFaddbegin \DIFadd{(coverage below 25)}\DIFaddend .
This is consistent with \DIFdelbegin \DIFdel{the observation that the main effect of low coverage is to
``break'' connectivity in }\DIFdelend \DIFaddbegin \DIFadd{previous observations where low coverage in sequences
cause ``breaks'' in connectivity within }\DIFaddend the assembly graph
\cite{Chaisson:2008p1373,Pevzner:2001p1374}.

To further evaluate the effects of partitioning, we introduced spiked\DIFaddbegin \DIFadd{,
simulated }\DIFaddend reads from \emph{E. coli} genomes into the \DIFdelbegin \DIFdel{mock community
}\DIFdelend \DIFaddbegin \DIFadd{HGMC }\DIFaddend dataset. First,
simulated reads
from a single genome (\emph{E. coli} strain E24377A, NC\_009801.1 with 2\%
substitution error and 10x coverage) were added to the \DIFdelbegin \DIFdel{mock community dataset and then processed in the
same way as the
unfiltered mock dataset.  We observed similar }\DIFdelend \DIFaddbegin \DIFadd{HGMC dataset and the
resulting dataset, HGMC.Ecoli1, was filtered, partitioned, and assembled as
described below. Similar }\DIFaddend amounts of data reduction after digital normalization
and partitioning (Table ~\ref{data-summary}) \DIFaddbegin \DIFadd{were observed}\DIFaddend . Among the 81,154
partitioned sets of reads \DIFdelbegin \DIFdel{, we
identified }\DIFdelend \DIFaddbegin \DIFadd{in the HGMC.Ecoli1 dataset, }\DIFaddend only 2,580 (3.2\%)
partitions \DIFdelbegin \DIFdel{containing }\DIFdelend \DIFaddbegin \DIFadd{contained }\DIFaddend reads from multiple genomes. \DIFdelbegin \DIFdel{A totalof }\DIFdelend \DIFaddbegin \DIFadd{In total, }\DIFaddend 424
partitions contained reads from the spiked \emph{E. coli} genome (201 partitions
contained \emph{only} spiked reads) and when assembled\DIFaddbegin \DIFadd{, these reads }\DIFaddend aligned to
99.5\% of \emph{E. coli} strain E24377A genome (4,957,067 of 4,979,619 bp)
(Supplementary Fig. 6).

Next, we introduced five closely-related \emph{E. coli} strains into \DIFdelbegin \DIFdel{the mock
community datasetand performed the same analysis. Partitioning this ``mix-spiked" mock community
datasetresulted }\DIFdelend \DIFaddbegin \DIFadd{original
HGMC dataset. This dataset, referred to as HGMC.EColi5, was filtered,
partitioned, and assembled, resulting }\DIFaddend in 81,425 partitions\DIFdelbegin \DIFdel{, of which }\DIFdelend \DIFaddbegin \DIFadd{. Among these, }\DIFaddend 1,154
...
Fig. 7).\DIFdelbegin \DIFdel{We
independently assembled }\DIFdelend \DIFaddbegin \DIFadd{Independently assembling }\DIFaddend the partitions containing reads originating
from the
spiked \emph{E. coli} strains \DIFdelbegin \DIFdel{.  Among the resulting }\DIFdelend \DIFaddbegin \DIFadd{resulted in }\DIFaddend 6,076 contigs, all but three
\DIFdelbegin \DIFdel{contigs originated }\DIFdelend \DIFaddbegin \DIFadd{originating }\DIFaddend from a spiked \emph{E. coli} genome. The remaining three contigs
were more than 99\% similar to \DIFdelbegin \DIFdel{HMP mock }\DIFdelend \DIFaddbegin \DIFadd{available }\DIFaddend reference genomes (NC\_000915.1,
...
originating from multiple genomes \DIFaddbegin \DIFadd{found in the HGMC }\DIFaddend (Supp Fig. 8), and 3,075
contigs (51\%) could be aligned to reads which originated from more than one
spiked genome.
\DIFdelbegin \DIFdel{This result is comparable to the }\DIFdelend \DIFaddbegin 

\DIFadd{The assembly of the HGMC.Ecoli5 dataset was also performed without removal of
any reads (e.g., no digital normalization or partitioning). Comparing the
HGMC.Ecoli5 unfiltered and filtered assemblies, we found that the }\DIFaddend fraction of
contigs which are associated with multiple genomes \DIFdelbegin \DIFdel{in }\DIFdelend \DIFaddbegin \DIFadd{were similar. In }\DIFaddend the
unfiltered data set, \DIFdelbegin \DIFdel{where }\DIFdelend 66\% of 4,702 contigs \DIFaddbegin \DIFadd{were }\DIFaddend associated with spiked reads
\DIFdelbegin \DIFdel{contain reads that originate }\DIFdelend \DIFaddbegin \DIFadd{which could have originated }\DIFaddend from more than one \DIFdelbegin \DIFdel{spiked
}\DIFdelend genome.

\section*{Data reduction and partitioning enable the assembly of two soil
metagenomes} \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend We next applied \DIFdelbegin \DIFdel{these }\DIFdelend \DIFaddbegin \DIFadd{digital normalization and partitioning }\DIFaddend approaches
to the {\em de novo} assembly of two soil metagenomes. \DIFaddbegin \DIFadd{Unfiltered }\DIFaddend Iowa corn and
prairie datasets (containing 1.8 billion and 3.3 billion reads, respectively)
could not be assembled by Velvet in \DIFdelbegin \DIFdel{500 GB }\DIFdelend \DIFaddbegin \DIFadd{500GB }\DIFaddend of RAM. A 75 million reads subset of
...
(Supplementary Fig. 9). Applying the same filtering approaches as \DIFdelbegin \DIFdel{described
above}\DIFdelend \DIFaddbegin \DIFadd{used for the
HGMC dataset}\DIFaddend , the Iowa corn and prairie datasets were reduced to 1.4 billion
and 2.2billion reads, respectively, and after partitioning, a total of 1.0
billion
and 1.7 billion reads remained, respectively. \DIFdelbegin \DIFdel{Prefiltering
}\DIFdelend \DIFaddbegin \DIFadd{Pre-filtering }\DIFaddend used 300 GB of RAM
or less. \DIFdelbegin \DIFdel{The
}\DIFdelend \DIFaddbegin \DIFadd{Notably, the }\DIFaddend large majority of k-mers in the soil metagenomes are
...
Based on the \DIFdelbegin \DIFdel{mock community }\DIFdelend \DIFaddbegin \DIFadd{HGMC }\DIFaddend dataset, we estimated that above a sequencing depth of five,
...
containing \DIFaddbegin \DIFadd{a minimum of }\DIFaddend 10 million reads. Data reduction and partitioning were
...
assembly. Among \DIFdelbegin \DIFdel{the }\DIFdelend paired end reads, 95.5\% of the reads aligned concordantly. In
...
concordantly (Table ~\ref{read-map}). Based on \DIFdelbegin \DIFdel{these mappings, we calculated read
recovery in assembled contigswithin the soil metagenomes
}\DIFdelend \DIFaddbegin \DIFadd{the alignment of sequencing
reads to assembled contigs, we estimated the distribution of sequencing coverage
in
resulting assemblies }\DIFaddend (Fig. ~\ref{soilassemblycoverage}). Overall, there is
...
\DIFdelbegin \DIFdel{Among contigs, the presence of polymorphisms was examined by
identifying the
amount of consensus obtained by reads mapped
}\DIFdelend \DIFaddbegin \DIFadd{As the resulting assemblies are consensus sequences representative of the
unassembled dataset, we investigated the degree of variation (i.e.,
polymorphism) present among aligned reads to assembled contigs. }\DIFaddend (Supplementary
...
profiles but different taxonomy} \DIFdelbegin %DIFDELCMD < 
...
\DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{We acknowledge that the }\DIFaddend diversity and sequencing depth represented by the \DIFdelbegin \DIFdel{mock community
}\DIFdelend \DIFaddbegin \DIFadd{HGMC
dataset }\DIFaddend is extremely low compared to that of most environmental metagenomes;
however, it represents a simplified, unevenly sampled model for a metagenomic
dataset which \DIFdelbegin \DIFdel{enables }\DIFdelend \DIFaddbegin \DIFadd{allows for }\DIFaddend the evaluation of \DIFdelbegin \DIFdel{analyses }\DIFdelend \DIFaddbegin \DIFadd{novel approaches }\DIFaddend through the
...
loss of assembly. This strategic filtering \DIFdelbegin \DIFdel{takes
advantage of the }\DIFdelend \DIFaddbegin \DIFadd{normalizes the abundance of reads in
a dataset to a specific sequencing coverage. As there exists an }\DIFaddend observed
coverage ``sweet spot'' at which point sufficient sequences are present for
robust assembly (Supp Fig. 1)\DIFdelbegin \DIFdel{.  
The normalization of sequences }\DIFdelend \DIFaddbegin \DIFadd{, this effectively reduces the volume of the
dataset for assembly while removing errors introduced by extraneous reads.
Further, this normalization }\DIFaddend also resulted in more even coverage (Fig.
...
The specific effects of filtering varied depending on \DIFdelbegin \DIFdel{differences
between reference genomes. Variable }\DIFdelend \DIFaddbegin \DIFadd{characteristics of
genomes. We observed that variable }\DIFaddend abundance and conserved regions in
references had an impact on \DIFdelbegin \DIFdel{filtered assembly recovery }\DIFdelend \DIFaddbegin \DIFadd{the recovery of sequences in filtered HGMC datasets}\DIFaddend .
The
...
these genomes \DIFdelbegin \DIFdel{would }\DIFdelend appear as high coverage elements and \DIFaddbegin \DIFadd{consequently would }\DIFaddend be
removed, as evidenced by a large difference in the number of reads associated
with NC\_005008.1 in the unfiltered and filtered \DIFaddbegin \DIFadd{(normalized) }\DIFaddend datasets
(supplementary Fig. 2). \DIFdelbegin \DIFdel{Consequently, the }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend unfiltered dataset contained \DIFaddbegin \DIFadd{comparably }\DIFaddend more reads
spanning these repetitive regions \DIFdelbegin \DIFdel{.  This most
}\DIFdelend \DIFaddbegin \DIFadd{which }\DIFaddend likely enabled assemblers to \DIFdelbegin \DIFdel{extend the }\DIFdelend \DIFaddbegin \DIFadd{more
effectively extend the unfiltered }\DIFaddend assembly of these sequences\DIFdelbegin \DIFdel{and resulted in the observed }\DIFdelend \DIFaddbegin \DIFadd{, ultimately
observed as an }\DIFaddend increased recovery of these genomes in \DIFdelbegin \DIFdel{the unfiltered assemblies.
}\DIFdelend \DIFaddbegin \DIFadd{these assemblies.
}

\DIFaddend This result, though rare among \DIFdelbegin \DIFdel{the mock
reference genomes }\DIFdelend \DIFaddbegin \DIFadd{genomes in the HGMC dataset}\DIFaddend , identifies a
shortcoming of our approach, and indeed for most short-read assembly
approaches,related to repetitive regions and/or polymorphisms. \DIFdelbegin \DIFdel{For the soil metagenomesour }\DIFdelend \DIFaddbegin \DIFadd{We acknowledge
that in our
assembled soil metagenomes, }\DIFaddend data reduction may \DIFdelbegin \DIFdel{have caused some information loss in exchange }\DIFdelend \DIFaddbegin \DIFadd{cause information loss but
exchange this disadvantage }\DIFaddend for the ability to assemble previously intractable
data sets. \DIFdelbegin \DIFdel{Evaluation of the mock community }\DIFdelend \DIFaddbegin \DIFadd{Overall, evaluation of the HGMC }\DIFaddend dataset suggests that this
information loss is minimal \DIFdelbegin \DIFdel{overall }\DIFdelend and that our approaches result in a comparable
assembly whose abundance estimations are \DIFaddbegin \DIFadd{even }\DIFaddend slightly improved.

%\subsection{Partitioning effectively separates genomes for assembly}
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend Metagenomes contain many distinct genomes, which are largely disconnected from
each other but which \DIFdelbegin \DIFdel{sometimes }\DIFdelend \DIFaddbegin \DIFadd{often }\DIFaddend share sequences due to \DIFaddbegin \DIFadd{sequence }\DIFaddend conservation or
lateral transfer. Our \DIFdelbegin \DIFdel{prefiltering }\DIFdelend \DIFaddbegin \DIFadd{pre-filtering }\DIFaddend approach removes both common multi-genome
elements as well as artificial connectivity stemming from the sequencing
process\DIFdelbegin \DIFdel{.
%DIF <  @CTB cite artifacts
}\DIFdelend \DIFaddbegin \DIFadd{(}\cite{howeartifacts}\DIFadd{). }\DIFaddend As shown above on the \DIFdelbegin \DIFdel{mock data set}\DIFdelend \DIFaddbegin \DIFadd{HGMC dataset}\DIFaddend , the removal of
these
sequences does not significantly alter the recovery of reference genomes
through {\em de novo} assembly: the resulting assemblies of \DIFdelbegin \DIFdel{unpartitioned and
}\DIFdelend \DIFaddbegin \DIFadd{unfiltered, filtered,
and
filtered and }\DIFaddend partitioned datasets were nearly identical\DIFdelbegin \DIFdel{for the mock data. The
}\DIFdelend \DIFaddbegin \DIFadd{. Further, the }\DIFaddend large
...
\DIFdelbegin \DIFdel{We further examined }\DIFdelend \DIFaddbegin \DIFadd{To further examine }\DIFaddend the recovery of sequences through partitioning\DIFdelbegin \DIFdel{by
computationally spiking in }\DIFdelend \DIFaddbegin \DIFadd{,
computationally-derived sequences from }\DIFaddend one or more \emph{E. coli} strains \DIFdelbegin \DIFdel{before
applying filtering and partitioning}\DIFdelend \DIFaddbegin \DIFadd{were
amended to the HGMC dataset}\DIFaddend . When we spiked in a single \emph{E. coli} strain,
...
(Supplementary Fig. 8). This result is not \DIFdelbegin \DIFdel{unexpected, }\DIFdelend \DIFaddbegin \DIFadd{unique to our approaches, however, }\DIFaddend as
...
provide
a \DIFdelbegin \DIFdel{much-reduced }\DIFdelend \DIFaddbegin \DIFadd{much reduced }\DIFaddend subset of sequences to be targeted for more sensitive assembly
...
the
\DIFdelbegin \DIFdel{mock community }\DIFdelend \DIFaddbegin \DIFadd{HGMC }\DIFaddend dataset,this gain was small, reducing unfiltered assembly at 12 GB and 4
...
%DIF <  @@CTB somewhere we should drive home the number of independent observations
%DIF <  in assembly.
%DIF < @@ACH What do you mean by independent observations? The # of annotations?  Unique proteins?
%DIF < @ACH Suggest just throwing the MG-RAST Public link in, need metadata to release public
\DIFdelbegin %DIFDELCMD < 
...
prior to assembly. The strategies are generic and \DIFaddbegin \DIFadd{broadly }\DIFaddend applicable to any
...
metagenome assembly techniques going forward \DIFaddbegin \DIFadd{and provide the critical references
which will enable future investigations of complex environments}\DIFaddend .
